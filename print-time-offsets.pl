...
while (<>) {
	...
	$hc_t = mktime(0,$hc_minute,$hc_hour,$mday,$mon,$year);#,$wday,$yday,$isdst);
	#print "hc_t=$hc_t\n";
	...
	print sprintf("my time %ld %02d:%02d:%02d.%06d %04d/%02d/%02d\n",
		      $t, $hour, $min, $sec, $us, $year+1900, $mon+1, $mday);
	print sprintf("HC time %ld %02d:%02d:00", $hc_t, $hc_hour, $hc_minute);

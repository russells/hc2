...
\begin{center}
  ...
  \includegraphics[width=\textwidth]{HC-front-cover.png}
...
\begin{itemize}
\item \texttt{NORMAL} for a normal temperature\footnote{Normal is
  equivalent to 20$^{\textrm{o}}$C}
...
\includegraphics[width=\textwidth]{HC-demo-nobat.png}
\vfill
...
\item Do not leave in direct sunlight.  This will result in incorrect
  temperature readings.  For outside temperature readings, place the device
  in a Stevenson screen.\footnote{Stevenson screen not supplied.}
...
\vfill% Push the footnote to the bottom of the page.
...
\includegraphics[width=\textwidth]{HC-display-parts-nobat.png}
...
\includegraphics[width=\textwidth]{HC-buttons.png}
...
\item The display shows \texttt{Yr 2014} (or the current year), and the year
  will flash.

\item Press \ButtonUp{} or \ButtonDown{} to change the year.  You cannot
  set the year to before 2014.
...
\item If power is lost, the default starting date and time is 2014-03-07 at
  17:30.
...
\includegraphics[width=\textwidth]{HC-temp-cal-plus-nobat.png}
...
not drift more than a minute per year.  But if you wish to change its time
adjustment:
%
\begin{itemize}
...
\item You can change the adjustment from $-20$ seconds per day to $+20$
  seconds per day, in steps of 0.25 seconds per day.
...
\includegraphics[width=0.7\textwidth]{HC-max-MAX_T__.png}

\includegraphics[width=0.7\textwidth]{HC-max-SINCE__.png}

\includegraphics[width=0.7\textwidth]{HC-max-3_PM___.png}

\includegraphics[width=0.7\textwidth]{HC-max-5__HOTS.png}
...
High capacity lithium batteries should last for at least three years in
normal use.
...
$\pm 4.5$ HOTS/COLDS ($\pm 4.5$$^{\textrm{o}}$C --- Eww) \\
\hline

Time accuracy (with adjustment) &
$\pm 0.125$ seconds per day (about 45 seconds per year) \\
\hline

Time adjustment range & $\pm 20$ seconds per day \\
\hline
